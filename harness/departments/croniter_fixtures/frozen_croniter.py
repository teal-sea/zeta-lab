...
class croniter:
    ...
    def __init__(
        ...
        day_or_union: bool = False,
    ) -> None:
        if day_or_union and not day_or:
            raise ValueError(
                "day_or_union=True requires day_or=True: union semantics for "
                "day-of-month/day-of-week cannot be requested together with "
                "intersection semantics (day_or=False)"
            )
        self._ret_type = ret_type
        self._day_or = day_or
        self._day_or_union = day_or_union
    ...
    def _calc_next(self, is_prev: bool) -> datetime.datetime:
        ...
        if (expanded[DAY_FIELD][0] != "*" and expanded[DOW_FIELD][0] != "*") and self._day_or:
            ...
            if self._implement_cron_bug and (
                ...
                #
                # With day_or_union=True the split is *made* clean instead: the
                # day-of-month side is computed without the nth-weekday ('#')
                # constraint and the day-of-week side without the nearest-weekday
                # ('W') constraint, so each special form binds only to its own
                # field and the two sides form a true union.
                clean_split = not nth_weekday_of_month and not self.nearest_weekday
                if self._day_or_union:
                    clean_split = True
                ...
                try:
                    t1 = self._calc(
                        current, expanded, dom_side_nth, self.nearest_weekday, is_prev
                    )
                except CroniterBadDateError:
                    if not clean_split:
                        raise
                ...
                try:
                    t2 = self._calc(
                        current, expanded, nth_weekday_of_month, dow_side_nearest, is_prev
                    )
                except CroniterBadDateError:
                    if not clean_split:
                        raise
        ...
        return self._calc(
            current, expanded, nth_weekday_of_month, self.nearest_weekday, is_prev
        )
    ...
    def _calc(
        ...
        procs = [
            proc_year,
            proc_month,
            (proc_nearest_weekday if nearest_weekday else proc_day_of_month),
        ...
        while abs(year - current_year) <= self._max_years_between_matches:
            ...
            if not exists and (
                ...
                while not exists:
                    unaware_time = self._calc(
                        unaware_time, expanded, nth_weekday_of_month, nearest_weekday, is_prev
                    )
                    aware_time, exists = _add_tzinfo(unaware_time, now, is_prev)
            ...
            alternative_unaware_time = self._calc(
                alternative_unaware_time, expanded, nth_weekday_of_month, nearest_weekday, is_prev
            )
