...
3. **Mertens' second theorem** (`ZetaLean.Mertens.mertens_second_theorem`,
   ...
   `|S(N) − loglog N| ≤ 9` for every `N`.
4. **Turán's variance bound** (`sum_sq_dev_le`, `turan_variance`): combining
   1–3, `∑_{n ≤ N} (ω n − loglog N)² ≤ 93 · N · loglog N` whenever
   `1 ≤ loglog N`, hence eventually; the constant `93` is `9² + 9 + 3`,
   where `9` is Mertens' band, and it moves quadratically with it.  It
   replaces the `113 = 10² + 10 + 3` and `275 = 16² + 16 + 3` this file
   carried before, and the `5855 = 76² + 76 + 3` it first carried: nothing
   in the variance argument changed on any of those occasions, only the
   band it is fed.
...
`turan_variance`, with the explicit constant `93`. -/
...
/-- **Turán's variance bound, with the explicit constant `93`**: whenever
`1 ≤ loglog N`, `∑_{n ≤ N} (ω n − loglog N)² ≤ 93 · N · loglog N`.

The constant decomposes as `B² + B + 3` with `B = 9` the Mertens band of
`ZetaLean.Mertens.mertens_second_theorem`, so `93`.  The dependence on
that band is quadratic, which is why tightening Mertens is the whole
lever here: `B = 10` gave `113`, `B = 16` gave `275`, `B = 76` gave
`5855`.  The shape
...
theorem sum_sq_dev_le {N : ℕ} (hL : 1 ≤ loglog N) :
    ∑ n ∈ Finset.Ioc 0 N, ((omega n : ℝ) - loglog N) ^ 2
      ≤ 93 * (N : ℝ) * loglog N := by
  have hMert : |(∑ p ∈ Finset.Ioc 0 N with p.Prime, ((p : ℝ))⁻¹) - loglog N| ≤ 9 :=
  ...
  have hsq : (S - L) ^ 2 ≤ 81 := by nlinarith
  have hD2 : (N : ℝ) * (S - L) ^ 2 ≤ (N : ℝ) * 81 :=
    mul_le_mul_of_nonneg_left hsq hN0
  have hSu : (N : ℝ) * S ≤ (N : ℝ) * (L + 9) :=
...
/-- **Turán's variance estimate**, with the explicit constant `93`. -/
