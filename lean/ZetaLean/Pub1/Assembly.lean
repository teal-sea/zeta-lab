...
/-!
# Pub 1 strong closure: assembly

`strongClosureData_of_member` is the sharp statement of where this
formalization stands: **every** input of the principal theorem is discharged
except membership of the constructed sequence in the source-admissible class.

`sourceWindow_taper` then reduces that single remaining input to exactly three
analytic facts about `w`, and no others:

1. `w ∈ C²` on an open neighbourhood of `I` (the distributional identity
   `F₁'' = 2δ₀ + q` and the resulting formula for `w''`);
2. radial monotonicity of `w`, which `ZetaLean.Pub1.Concavity` derives from the
   strict concavity bound `w'' < -0.59326318` — itself waiting on the
   residual-derived `C²` certificate `‖w'' - u''‖_∞ < 0.006060899845`;
3. the uniform `L¹` bounds on `φ_L''` and `(φ_L²)''`.

Positivity of `w` on the neighbourhood follows from `1/5 ≤ w` on `I` together
with (1); it is listed separately below only because it is stated on the larger
interval.
-/
...
/-- **The remaining obligation, in one statement.**  If the taper is a source
window at every `L ≥ 8`, the strong-closure theorem holds outright. -/
