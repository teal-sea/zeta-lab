...
- **Invalidated by:** running it against a shallow clone. `.github/workflows/
  ...
  clock for one day.
- **Now caught by:** two changes. `tests.yml` and `full.yml` check out with
  `fetch-depth: 0`, as `checks.yml` already did for `scripts/70_lab_state.py`.
  And the test now asks `git rev-parse --is-shallow-repository` first and
  **skips with a reason** rather than comparing against a meaningless date,
  which is `proven_sign` returning 0 for "not decided" rather than guessing.
  Verified by cloning this tree at `--depth 1`: the old test reproduces the CI
  failure there, the new one skips.
