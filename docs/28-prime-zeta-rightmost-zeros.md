...
- **The OEIS correction is drafted, not posted.**
  ...
  Both now need rewriting to cite Belovas et al. for the threshold, which
  is what the hunt's own kill condition required; posting anything to OEIS
  is an operator action, explicitly outside the hunt's permissions.
