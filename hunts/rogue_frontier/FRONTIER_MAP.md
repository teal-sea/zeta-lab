...
- gap: The lab has already transplanted a Cheer–Goldston-type gap-census floor into the 2026 unconditional framework (0.6725007037 → 0.6725106958; PRIOR ART, not to be re-proposed). This has now been surpassed by Ainta's explicit finite certificates over larger configurations (up to 7-point configurations yielding 0.673008528). The remaining question is whether stronger census invariants (multi-window inertia rather than nearest-neighbor counts) or larger configurations reach further toward 0.68185 without exponential explosion.
...
- statement: Unconditionally, at least 67.250% (constant 0.6725007037) of the nontrivial zeros of ζ are simple zeros on the critical line (also: distinct zeros on the critical line), removing the narrow-box restriction from the BGST framework. Accompanied by a Lean 4 formalization with main theorem files free of sorrys. The manuscript itself notes the technique averages over zeros and is unlikely to reach 100%. Improved explicitly by Ainta's finite certificates over consecutive zeros to 67.3008528% (7-point configuration).
- constant: 0.673008528 (Ainta finite certificate on 7 consecutive zeros; 3-point gave 0.672519767; lab's prior art 0.6725106958 via Cheer-Goldston gap-census floor)
- status: unconditional | Claude (Anthropic) (2026), finite certificates by Sunghyeon Jo (ainta) (2026)
- citation: Anthropic preprint, https://www-cdn.anthropic.com/564f962e60643842f5fcb4a17c9dbc8f608f1c37.pdf; ainta/zeta-simple-zeros
- checked against: https://github.com/ainta/zeta-simple-zeros (reproduced locally: finite certificate verification passed)
- technique: Unconditional pair-correlation input (BGST lineage) + removal of the box hypothesis + stability refinement of rank–trace inequalities (Ainta) verified via interval arithmetic
- bottleneck: The averaging structure of pair correlation cannot exclude sparse exceptional sets; each further digit comes from sharper test-function or gap-census inputs, expanding the finite configuration size (e.g. 7-point exhaustive subdivision takes minutes, higher points scale badly)
- tunable: Admissible test functions under the framework's (unconditional) constraint set; gap-census floors; joint simple/critical functionals; finite certificate sizes
- gap: The Chirre-Gonçalves-de Laat SDP function class has not been transplanted into this framework; Cohn-Elkies-class functions could plausibly move the third decimal. Ainta's bounds rely on interval arithmetic and subdivision; a closed-form SDP bound over the multi-point configuration might avoid the exponential subdivision cost.
