...
| transplant-lemma (Fable) | the near-coincident closure campaign: four parallel angles on the last quantifier | `repulsion_trade.py`, `near_coincident.py`, `exact_gap_attack.py`, `verify_lemma_c_independent.py`, `RETENTION-PROBLEM.md` (+ tests) | **BLOCKER 2 CLOSED at hardened grade** — three routes agree, exact-rational certificate, coordinator reproduction; see ledger 2026-08-13. `cluster_sdp.py` still ACTIVE |
