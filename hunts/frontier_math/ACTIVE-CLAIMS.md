...
|---|---|---|---|
| codex-counting-lattice | re-audit the large-k budget floor after re-optimising the critical-lattice spacing at every k; derive the `2*pi` lattice limit | `counting_lemma.py`, `test_counting_lemma.py`, `PROOF-LEDGER.md` | **ACTIVE** — fixed-spacing `6.3e-3` floor falsified; exact Poisson target `c2(0) - A^2 = 0.00517169408367867955` identified, hardening in progress |
