...
| lab-rejection-philosophy | O9 cost estimate: how large the nine-window damage table is as an interval object | `o9_scoping.py`, `test_o9_scoping.py`, `O9-SCOPING.md` | **LANDED** 2026-08-13 — measures only, lands nothing in `zeta23ext`. Headline: the recorded `c_k` are attained suprema, so O9 as written has zero margin and cannot close under enclosures; §7 absorbs up to a **1.3945x** cap inflation, and at 1.20x inflation + 0.02 window widening the whole object is **264 leaves, depth 12**, about a third the size of `BandCert/Data.lean`. Size was never the obstacle |
