...
| zeta-constant-improvement | **standing up Lean at all**, then O9 as a two-variable table | `zeta23ext/Zeta23Ext/{EForm2/Bridge,EForm2/Estimates,EForm3/Numerics,TruncEst/Kernel,TruncEst/Decay,TruncEst/Autocorrelation,RetentionWired}.lean` (drift fixes), `EForm3/O9{Field,Data2,Check2}.lean`, `o9_leaf2d.py` (+ tests), `RETENTION-PROBLEM.md` §4 | **DONE** 2026-08-14 — first session in this hunt with a Lean toolchain, so `zeta23ext` had **never been compiled**. It did not build: seven modules in the root's import closure fail, in three cascading layers (each layer only reachable once the one before it compiled). All seven are Mathlib drift, not mathematics — `convert … using 1` overshooting into instance-equality goals, bare `simp only` beta-reductions, `HasDerivAt.mul` concluding about `c * d` rather than a lambda, and `sin_gt_sub_cube` dropping its `x ≤ 1` hypothesis. Fixed with `HasDerivAt.congr_deriv` on defeq statements. One of them, `RetentionWired.margin_identity`, was carrying a **`sorryAx`** induced by its own build error. **`EForm3/Numerics` is on the k=1 retention chain.** Separately: O9 rebuilt as a genuine 2-D table needing **no** depth-reduction lemma — `1939` leaves, depth 20, 0 undecided, smallest carrying margin `1.05e-08`, and **`decide +kernel` accepts all 49 chunks** (211 s, axioms `[propext]`). A first attempt at `598` leaves was **rejected by the kernel on 14 of 15 chunks**: it predicted the verdict with Arb leaves, on `o9_leaf.py`'s LEAF CAVEAT that Arb and `Leaves.lean` agree to `2^-60`. **That caveat is false on wide cells** — Arb encloses by range, `sinCosIv` evaluates a Taylor interval and applies `dbl` twice, and `dbl` squares. `o9_leaf2d.py` now mirrors Lean's algorithms bit-for-bit. **`o9_leaf.py`'s 344-cell 1-D table rests on the same false caveat and has never met the kernel; assume its count is low by a similar factor.** Of the two membership lemmas, **O9a (`shfnIv_mem`) is proved** (it needed a sharper `sinh` remainder than `Leaves.sinh_taylor` provides, since `shfunR` divides by `t`); **O9b (`o9Field_mem`) is open**, with no `sorry` standing in |
