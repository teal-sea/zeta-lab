...
| zeta-constant-improvement | **standing up Lean at all**, then O9 as a two-variable table | `zeta23ext/Zeta23Ext/{EForm2/Bridge,EForm2/Estimates,EForm3/Numerics,TruncEst/Kernel,TruncEst/Decay,TruncEst/Autocorrelation,RetentionWired}.lean` (drift fixes), `EForm3/O9{Field,Data2,Check2}.lean`, `o9_leaf2d.py` (+ tests), `RETENTION-PROBLEM.md` §4 | **ACTIVE** 2026-08-14 — first session in this hunt with a Lean toolchain, so `zeta23ext` had **never been compiled**. It did not build: seven modules in the root's import closure fail, in three cascading layers (each layer only reachable once the one before it compiled). All seven are Mathlib drift, not mathematics — `convert … using 1` overshooting into instance-equality goals, bare `simp only` beta-reductions, `HasDerivAt.mul` concluding about `c * d` rather than a lambda, and `sin_gt_sub_cube` dropping its `x ≤ 1` hypothesis. Fixed with `HasDerivAt.congr_deriv` on defeq statements. One of them, `RetentionWired.margin_identity`, was carrying a **`sorryAx`** induced by its own build error. **`EForm3/Numerics` is on the k=1 retention chain.** Separately: O9 rebuilt as a genuine 2-D table (`598` leaves, depth 18, 0 undecided, smallest carrying margin `1.85e-08`) which needs **no** depth-reduction lemma; its two membership lemmas (O9a/O9b) are stated and **not** proved, with no `sorry` standing in |
