...
| The joint cap (on-line configuration-free) | DELIVERED (measured grade) | three-zone split + taper + LAW M mean + LAW N leak cap + cell C-S; v2 per-cell mid margins | pinch (nu 1.5, y 0.49): cap 37.36 vs budget 41.11, **margin +3.75**; greedy reaches 5.00 (extended greedy 5.56); theta = 1 cap infinite; dominates every level-3/4 measured adversary |
...
| **The seam** nu_p in [1.1, 1.4] deep | **PINNED, OPEN** | first pair spike shoulder (2 pi nu_p ~ 8) re-enters the band top | both bounds fail (v-cap -4.6..-19.1 abs; separable -0.17..-0.60) while the extended greedy extracts 7.23 of 35.1 at nu 1.3; escaping band `v in [5, 7]` exactly (band share 17.8 of the 30.8 M_II term); span boundary: span >= 8 closes at nu 1.5, span 6 open; THE next kill control |
