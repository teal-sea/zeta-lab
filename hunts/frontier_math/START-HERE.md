...
* **O9 is now a two-variable table needing no depth-reduction lemma**
  ...
  kernel-checked over the whole rectangle. Its two membership lemmas — the
  seam to `Dam` — are stated and **not** proved, with no `sorry` standing
  in, so the kernel has checked a table and not yet an inequality about the
  damage.
