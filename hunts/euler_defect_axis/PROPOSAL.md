...
the knob" wants replacing with the corrected axis and the observation that the
family has no knob: over the nine loud discriminants the defect band is 2.96
to 5.08, and it correlates with `log|d|` at `-0.82` and with class number at
`-0.69`, both negative.

## 5. Disposition of the compute job

The off-line-zero measurement issue #93 prices is not worth buying against
this axis. If the question is worth pursuing, the family to build is a
