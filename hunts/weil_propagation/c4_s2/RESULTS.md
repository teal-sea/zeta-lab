...
pushed. The five lines above are the mission's verdict. Every mathematical
...
RESULTS, not pinned by tests. Grades follow the `AGENTS.md` ladder, and a
...
3. **Information class:** both compute proposals stay inside the data the
...
**Compute proposals (Modal/Namespace), not run.** They exceed the local
limit, and the operator's standing rule excludes GitHub Actions and any
push, so a follow-up, if approved, runs outside this mission. two_adic/
§7b: nvec 140 to 200 at S = 4800 with an S = 9600 check, about 40 CPU
minutes. checker/ §7.6: the (240, 2400, 32) unit and the default-rule rows
at N = 32, checkpointed per unit (`run_checker_ts.py --units`). The
200-mode N = 32 unit took 247 s; the 240-mode unit passed 9.2 CPU-minutes
without finishing, so the job should time one unit first. (The worker
folders' RESULTS call these "CI proposals"; the substance is the same.)
