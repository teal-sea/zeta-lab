1. **T_S was built for ζ on all nine cells, in the module form, not the product-ball form the mission first specified.** The product-ball Π_S of theory §7.3 defines no operator on X_S, where Connes' eq. (22) lives, and its literal formula has infinite trace there (cutoff/ and two_adic/ independently; ordinary argument, unreviewed). The built term is T_S = T_∞ + ΔT on the CCM module form (arXiv:2310.18423 Thm 4.6, used as published): T_∞ from kernel/'s S_∞, which needs no numerically identified prolate data (derivation unreviewed, numerics hardened), and ΔT from two_adic/ in float64 with an error band of 5.3e−3 to 1.6e−2 per (c, N) row at N = 8 and 16, and **3.0e−2 / 1.4e−2 / 8.2e−3 at N = 32** (c = 2.2 / 2.5 / 2.9). ΔT's explicit Gram inverse was replaced on 2026-09-24 by a QR of the Gram factor (two_adic/ c3dca00): the ill-conditioning came from the s cutoff, not from the functions, and on a test case at cond(G) = 2.6e15 the new route matches a 256-bit reference to 6.8e−9 where the old one was off by 1.6e−2. Under it no eigenvalue of T_S lies below −band on any row, including the 319 and 364-mode rows at N = 32, which the old route broke by 14 to 37 (checker/ s7.8). The band is not an error estimate in principle either: the mode-truncation error of ΔT is a cancellation between two traces that are each +∞, and that ΔT^(n) converges to ΔT_exact as the mode count grows is proven nowhere in the record (bound_trunc/ Lemma 1, ordinary argument, passed referee/'s internal review; the log-n growth of one of the two traces, on one window at c = 2.9, hardened on two routes). **Grade: measured, weakest step ΔT, whose limit in the mode count is an unproven assumption of every statement about the exact T_S and R_S.**
2. **R_S = Q − T_S measures how much of the 2-adic atom the trace term absorbs: all of its depth. Its negative count at c = 2.9 grows 4, 10, 20 at N = 8, 16, 32; at 2.5 and 2.2 it does not grow at N = 32.** λ_min moves from −0.30 to −0.49 (product side, Q − T_∞) to −0.026 / −0.039 / −0.097 to −0.12 at c = 2.2 / 2.5 / 2.9. The count below −band is 4, 4 / 4, 9 / 4, 10 at N = 8, 16. At c = 2.9 the rise from 4 to 10 holds at both 120 and 160 prolate modes, on a last pair 1.4 to 1.5 times the band; at 2.2 and 2.5 the N = 16 count falls as modes rise. At N = 32, under the QR rho and by the reading checker/ committed before any rebuilt row was analysed (49db49f): at c = 2.9 the count is **20** on the delivered row and on the 240, 280 and 319-mode rows (21 at 364), the band is two_adic/'s probe (8.2e−3), no refinement moves T_S by more than 4.95e−3, and none of the 12 top-half negatives (−1.45e−2 to −1.21e−2) is shallower than a response. At 2.5 the refined rows count 6 and 5 against 9 at the band 1.4e−2 set by the 319-mode row; at the 240-mode row's band (1.1e−2) they would count 14 and 12, so the 2.5 verdict rests on the band rule (a sensitivity computed after the numbers, labelled as such). At 2.2 every build counts 0. C4's bounded-rank prediction is **not supported at c = 2.9**: the count grows with N at every resolution tried there. That is measured evidence against it, not a refutation: ΔT is measured grade, its band indicates and does not bound, and three values of N are not a limit. (The first build's reading of N = 32 as undecided, from the explicit inverse, is kept in checker/ s7.7.) Exact inertia of the eight stored c = 2.9 matrices (checker/ s7.9, bee66ba) confirms the counts 4, 10, 20 on every build, each counted eigenvalue strictly below −band; but 14 of the 20 at N = 32 lie between −2 band and −band, so the N = 16 to 32 growth does not survive at 2x band, while the N = 8 to 16 growth does (4 to 8). **Grade: hardened on the stored matrices; as a statement about R_S, measured, weakest step ΔT.** **Error bound on ΔT (delta_t_bound/, 2026-09-24): none usable, outcome 4.** No bound on ‖ΔT_exact − ΔT_stored‖ closes on any of the 33 builds: the mode truncation is blocked by the Sonin overlap on the prolate tail (bound_trunc/), and the Gram under the s cutoff by ‖X‖ ≥ 70 (bound_quad/ E2, closable by an exact-Gram arb recomputation, estimated at about 200 core-hours for the 280-mode N = 32 build alone, not run); both unresolved, not obstructed. So no count below −ε exists, and 4, 10, 20 keeps the grade above. A closed bound would have to stay below 1.18e−2 for the 20 at N = 32 to survive, and bound_quad/'s one closed source is already 9.2e−3 to 1.46e−2 there. What survives is mode-free: T_S ≥ (√2 − 1)⁴ T_∞ (bound_trunc/ Lemma 3, ordinary argument, passed internal review) gives n_−(R_S) ≥ 2, 2, 2 at c = 2.9 (N = 8, 16, 32), 1, 1, 1 at 2.5 and 0 at 2.2 on the full space, and 0 on C4's class V_4 at every cell (exact inertia on dps-40 inputs, reproduced by referee/ on a second route). That floor is flat and does not bear on the 4, 10, 20 count. **Grade of the follow-up: outcome 4; the floor is enclosure-carrying on inputs of hardened accuracy, conditional on Lemma 3, reviewed internally and not externally.**
...
4. **Open:** (a) ΔT accurate below Q's smallest eigenvalue (2.6e−4 at c = 2.2, N = 8): **not met at any mode count from 80 to 200** at S = 4800; no band falls below 6.5e−4, and two_adic/'s extrapolation of 0.36 per 20 modes is refuted (measured ratios 0.53 to 0.80). (b) The c = 2.9 growth is measured, not bounded: turning it into a statement needs ΔT with an error bound rather than a response band; delta_t_bound/ tried and none closes (line 2). Still open there: a bound on the Sonin overlap for the truncation, and for E2 either the exact-Gram recomputation or a frequency-weighted perturbation argument (a referee/ thread, not a claim). The ΔT-bound follow-up launched no Modal run (0 USD). The c = 2.5 count at N = 32 depends on which refined row sets the band. The float64 inverse that blocked N = 32 is fixed (two_adic/ c3dca00). Modal, 2026-09-24, with operator approval: the two compute proposals (0.741 USD; (a) did not meet its target, the first N = 32 build did not decide the count) and the rebuild of all eleven checker/ units under the fixed rho (0.57 USD), 1.31 USD of a 25 USD cap (modal/). (c) A proof: module-form C4 needs a semilocal analogue of Connes-Consani's ε(ρ), which does not exist yet (cutoff/). (d) The Dedekind control needs 23 in S, outside this mission.
5. **ALIGNMENT §5: theory §7.3 item 2 refuted** (exact local fact at 2); **product-side C4 refuted** on every c ∈ (2, 3) (ordinary argument, unreviewed; the growing negative index measured by cutoff/ and checker/ independently); **gap (a) closed as a construction**; **module-form C4 at S = {∞, 2} unresolved**, with measured evidence against its bounded-rank prediction at c = 2.9 (n_− = 4, 10, 20 at N = 8, 16, 32); **the error bound on ΔT unresolved** (paused by the box and by allocation, not obstructed; delta_t_bound/). Nothing here is a claim about RH; positivity on these windows is already known (Zhu, arXiv:2608.24827).
