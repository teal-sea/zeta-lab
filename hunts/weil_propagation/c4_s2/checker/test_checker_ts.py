...
def test_what_sets_the_band():
    """RESULTS s7.3: the refinement response (the N = 16 proxy at N = 32)
    sets the band on every N = 8 row and at 2.2 for N = 16, 32; the Gram
    probe sets it at 2.5 and 2.9 for N = 16, 32."""
    J = _cells()["cells"]
    for c in CELLS:
        for N in ("8", "16", "32"):
            x = J[c][N]
            ref = x["refinement_response"] if N != "32" else x["refinement_proxy"]
            by_ref = N == "8" or c == "2.2"
            assert x["band"] == (ref if by_ref else x["probe"]), (c, N)
