# Brief: bound_quad/, the quadrature, s cutoff, Gram and rounding errors of ΔT

You own `hunts/weil_propagation/c4_s2/bound_quad/` and write nowhere else
...
Bound every error of ΔT_stored except the mode truncation, for a fixed mode
count nvec: the s cutoff S (with the order-1/S tails), the w quadrature
(Gauss panels on [1, 2^Kmax], the dyadic truncation at Kmax, the asymptotic
1/w tail series and its term count), the float64 mode data (η_n, ζ_n), the
Gram step (`ta_mellin.rho`, QR since two_adic/ c3dca00), and float64
rounding in the assembly of the stored matrix. The target is
‖ΔT_nvec,exact − ΔT_stored‖ in spectral norm on the (2N+1)-dimensional
space, where ΔT_nvec,exact is the same finite-mode object computed without
any of these errors.

Two routes, pick by cost: (i) an a priori bound per source via arb
enclosures of the integrands and derivative bounds for the Gauss rules, or
(ii) an arb recomputation of ΔT_nvec itself as a ball matrix, giving
eps = ‖center − stored‖ + radius. Route (ii) over about 140,000 w nodes and
up to 364 modes will not fit locally: estimate one unit first (ask the
coordinator, then append the estimate to `modal/RUNS.md`) before any batch.
Mixed routes are fine; say which source each piece of your bound covers.

## Milestones

1. **Plan, within 45 minutes.** `DERIVATION.md` section 1: the list of
   sources above, for each the route and whether it closes, and a cost
   estimate from one measured unit. Commit; status message to the
   coordinator.
2. **Derivation**, every assumption numbered. Commit and tell the
   coordinator: it goes to referee/.
3. **Enclosure code.** `eps_quad.py`, `eps_quad.json` in the interface below,
   for every stored build at c ∈ {2.2, 2.5, 2.9}; `INTERFACE.md`; tests in
   `test_bound_quad.py`, including that the bound dominates the measured S
   responses (two_adic/ s7b: 6.5e−4, 1.1e−4 at 80 modes, 1.8e−4 at 160) and
   the QR-against-256-bit comparison of two_adic/ s10.3.
4. **RESULTS.md**, first five lines: the bound per source, its values on the
   c = 2.9 builds, its grade, what is open.
