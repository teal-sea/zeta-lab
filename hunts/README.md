...
### Hunt #77: Krenn-Gu 8x3: bounded unrestricted step (`r_abf47b/`)
**Status: not-settled.** Reproduced the cap-3 (4,500) and cap-4 (4,216) residuals on the exact Support-7 system. Proved that the 57 pair orbits merely refine the 31 triple orbits (adding no branches), making the inherited proposal to scale to 57 skeletons redundant. Constructed the 6561-equation, 138-variable exact polynomial system for Support 7 (torus factors removed) and priced an unrestricted ideal membership check using `sympy.groebner` with `grevlex`, which timed out at 5 minutes, establishing the infeasibility of this exact representation without a compiled backend.
