...
`hCert`.  This package discharges `hCert` at `n = 4`, so that the `n = 4` instance of that
theorem becomes **unconditional**.

Sections 1–6 are `hunts/ainta_seven_point/lean-three-point/ThreePoint/Base.lean` with the
namespace changed and three anchor lemmas added: the four-point pressure cutoff is `5.775`
against the three-point `4.035`, so the table's cells reach `6.25` and need the half-integer
anchors `5`, `11/2` and `6` that the three-point table never used.
...
/-! ### Anchor values.  `cos (π a)` and `sin (π a)` at the twelve half-integers the table uses. -/
...
lemma cs_5 : Real.cos (Real.pi*(5:ℝ)) = -1 ∧ Real.sin (Real.pi*(5:ℝ)) = 0 := by
  constructor <;> · rw [show Real.pi*(5:ℝ) = Real.pi + 2*Real.pi + 2*Real.pi by ring]
                    simp [Real.cos_add, Real.sin_add]

lemma cs_h11 : Real.cos (Real.pi*(11/2:ℝ)) = 0 ∧ Real.sin (Real.pi*(11/2:ℝ)) = -1 := by
  constructor <;> · rw [show Real.pi*(11/2:ℝ) = Real.pi/2 + Real.pi + 2*Real.pi + 2*Real.pi by ring]
                    simp [Real.cos_add, Real.sin_add]

lemma cs_6 : Real.cos (Real.pi*(6:ℝ)) = 1 ∧ Real.sin (Real.pi*(6:ℝ)) = 0 := by
  constructor <;> · rw [show Real.pi*(6:ℝ) = 2*Real.pi + 2*Real.pi + 2*Real.pi by ring]
                    simp [Real.cos_add, Real.sin_add]
